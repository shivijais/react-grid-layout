...
const compactEmptyRows = layout => {
  const sorted = layout.slice(0).sort((a, b) => a.y - b.y);
  const rows = sorted.reduce((acc, item) => {
    let currentRow = acc[acc.length - 1];
    const addNewRow = !currentRow || currentRow.top !== item.y;
    if (addNewRow) {
      currentRow = {
        top: item.y,
        bottom: 0,
    ...
    currentRow.tiles.push(item);
    if (item.y + item.h > currentRow.bottom) {
      currentRow.bottom = item.y + item.h;
    }
    return acc;
  }, []);
  ...
  let lastRowBottom = 0;
  rows.forEach(row => {
    const offset = row.top - lastRowBottom;
    if (offset > 0) {
      row.tiles.forEach(tile => (tile.y -= offset));
      row.bottom -= offset;
    }
...
export function moveElement(
  ...
  if (collisions.length > 0) {
    const movingUp = oldY > l.y;
    if (!movingUp) {
      const offset = l.y - oldY;
      const impactedTiles = sorted.filter(item => item.y >= oldY + l.h);
      impactedTiles.forEach(item => (item.y += offset));
    }
  }

  compactEmptyRows(layout);
